...
const int starter=8;
const int VLD_MOTOR = 2;
const int VLD_MOBIL = 4;

bool motor;
bool mobil;

const int RELAY_MOTOR = 6;
const int RELAY_MOBIL =7;
...
void setup() {
  ...
  pinMode(VLD_MOTOR, INPUT_PULLUP);   // Enable pin's pull up resistor
  pinMode(VLD_MOBIL,INPUT_PULLUP);
  pinMode(RELAY_MOTOR,OUTPUT);
  pinMode(RELAY_MOBIL,OUTPUT);
  digitalWrite(RELAY_MOTOR,HIGH);
  digitalWrite(RELAY_MOBIL,HIGH);
  ...
  Serial.begin(9600);
  Serial1.begin(9600);
  Serial2.begin(9600);  // Initialize serial communications with PC
...
///////////////////////////////////////// Main Loop ///////////////////////////////////
void loop () {
    int sensorRelayMotor = digitalRead(VLD_MOTOR);
    int sensorRelayMobil = digitalRead(VLD_MOBIL);
    while(Serial1.available()){
    delay(10);
    char cm = Serial1.read();
    dataMobil += cm;
    mobil = true;
    digitalWrite(RELAY_MOBIL,LOW);
  }
  ...
  //   }
  //   else {
  //     // if ( motor == true ) { // If not, see if the card is in the EEPROM
  //     // digitalWrite(buzzer,HIGH);
  //     // delay(250);
  //     // digitalWrite(buzzer,LOW);
  //     // // delay(500);
  //     // // motor = false;
  //     // }
  //     // if(mobil == true) {      // If not, show that the ID was not valid
  //     //     successWrite();
  //     // }
  //   }
  // }
...
void normalModeOn () {
  ...
  digitalWrite(greenLed, LED_OFF);  // Make sure Green LED is off
  //digitalWrite(kontak, HIGH);    // Make sure Door is Locked
}
